...
const Header = () => {
	...
	<div className="flex items-center h-14 w-screen z-1 " >
		<AiOutlineMenu className="ml-8 text-3xl " onClick={() =>
		    dispatch(toggleMenu())
	    } />

	    <Link to='/' className="flex justify-center items-center col-span-1 ml-12 text-2xl">
		<FaYoutube />
		...
		<div className="w-1/2">
		    ...
		    {showSuggestion && <ul className="absolute w-[41.5%] z-10 bg-white rounded-2xl mx-1 " >
			{
			    ...
			    <Link to={"/searchpage?q="+searchQuery} className="text-2xl px-6 h-10 flex items-center rounded-r-full border-y-2 border-l-2 border-gray-300 bg-gray-200">
				<IoIosSearch />
			    </Link>
			</div>
